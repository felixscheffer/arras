...
public class MusicLibraryParser {
    ...
    private class Handler extends DefaultHandler {
        ...
        protected void push(int newState, boolean ignoreCharacterData) {
            Item item = new Item(this.state, ignoreCharacterData);
        ...
        private void endCollectValue(String element) {
            ...
            if (this.key.equals("Track ID")) {
                this.track.setId(Long.parseLong(value));
            }
            ...
            if (this.key.equals("Artist")) {
                this.track.setArtist(value);
                return;
            }

            if (this.key.equals("Album")) {
                this.track.setAlbum(value);
                return;
            }

            if (this.key.equals("Genre")) {
                this.track.setGenre(value);
                return;
            }

            if (this.key.equals("Play Count")) {
                this.track.setPlayCount(Integer.parseInt(value));
                return;
            }

            if (this.key.equals("Rating")) {
                this.track.setRating(Integer.parseInt(value));
                return;
            }
        ...
        public InputSource resolveEntity(String publicId, String systemId) throws SAXException {
            if (publicId.equals("-//Apple Computer//DTD PLIST 1.0//EN")) {
                InputStream local = new BufferedInputStream(getClass().getResourceAsStream("PropertyList-1.0.dtd"));
    ...
    public List<Track> parseTracks(URL resource) {
        ...
        this.logger.info(format("Parsing music library %s", resource));
        ...
        this.logger.info(format("Parsed %d tracks in %d ms", result.size(), elapsed));
